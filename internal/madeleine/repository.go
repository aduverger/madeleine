...
func ResolveRepository(ctx context.Context, path string) (Repository, error) {
	...
	return Repository{WorktreeRoot: worktreeRoot, GitCommonDir: gitCommonDir, Origin: origin}, nil
}

func (s *Service) ResolveRepository(ctx context.Context, path string) (Repository, error) {
	repository, err := ResolveRepository(ctx, path)
	if err != nil {
		return Repository{}, err
	}
	if err := s.database.WithTransaction(ctx, func(transaction *store.Tx) error {
		return persistRepository(ctx, transaction, &repository)
	}); err != nil {
...
func persistRepository(ctx context.Context, transaction *store.Tx, repository *Repository) error {
	...
	for _, alias := range aliases {
		repositoryID, found, err := transaction.FindRepositoryIDByAlias(ctx, alias.kind, alias.value)
		if err != nil {
			return err
		}
		if !found {
		...
		if matchedRepositoryID != "" && matchedRepositoryID != RepositoryID(repositoryID) {
			return fmt.Errorf("%w: repository aliases match different repositories", ErrConflict)
		}
		matchedRepositoryID = RepositoryID(repositoryID)
	}
	...
	if matchedRepositoryID == "" {
		...
		if err := transaction.InsertRepository(ctx, store.RepositoryRecord{ID: string(repositoryID)}, createdAt); err != nil {
			return err
		}
	} else {
	...
	for _, alias := range aliases {
		...
		if err := transaction.InsertRepositoryAlias(
			ctx, string(repository.ID), alias.kind, alias.value, createdAt,
		); err != nil {
			return err
		}
	}
