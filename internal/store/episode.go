...
const episodeSelect = `
	SELECT e.id, e.source_capture_id, e.repository_id, e.conversation_id,
		conversation.external_id, e.harness, e.l1, e.l2, e.transcript_id,
...
func (tx *Tx) InsertEpisode(ctx context.Context, episode EpisodeRecord) error {
	_, err := tx.tx.ExecContext(ctx, `
		INSERT INTO episodes(
			id, source_capture_id, conversation_id, repository_id, transcript_id,
			harness, started_at, ended_at, l1, l2, created_at
		) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		episode.ID, episode.CaptureID, episode.ConversationID, episode.RepositoryID,
		episode.TranscriptID, episode.Harness, timestamp(episode.StartedAt),
		timestamp(episode.EndedAt), episode.L1, episode.L2, timestamp(episode.CreatedAt))
	return err
}
...
func scanEpisode(source scanner) (EpisodeRecord, error) {
	...
	if err := source.Scan(
		...
		&episode.TranscriptID, &startedAt, &endedAt, &createdAt,
	); err != nil {
